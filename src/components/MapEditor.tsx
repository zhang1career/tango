  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <textarea value={rules} onChange={(e) => setRules(e.target.value)} onBlur={commit} style={{ ...inputStyle, minHeight: 50 }} />
      </div>
      <div style={{ marginBottom: 10 }}>
  ...
  return (
    <div style={{ background: '#1e1e32', borderRadius: 8, padding: 14, border: '1px solid #444' }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <input value={condition} onChange={(e) => setCondition(e.target.value)} onBlur={commit} style={inputStyle} placeholder={'$items has "令牌"'} />
      </div>
            ...
            <div style={{ width: 260 }}>
              ...
              onUpdate={(data) =>
                setEdges((eds) =>
                  eds.map((e) =>
                    e.id === selEdge.id
                      ? { ...e, data: { ...e.data, ...data }, label: (data as { displayText?: string }).displayText ?? e.label }
                      : e
                  )
                )
              }
