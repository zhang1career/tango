...
function CollapsibleSection({
  ...
  return (
    <div style={styles.collapsible}>
      <div style={styles.collapsibleHead} onClick={onToggle} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && onToggle()}>
        <span>{title}</span>
        <span>{expanded ? '▼' : '▶'}</span>
      </div>
...
const COLLAPSE_KEYS = ['attr', 'inv', 'rules', 'onMeetAttr', 'onMeetItems'] as const;
    ...
    <div>
      ...
      <CollapsibleSection title="属性" expanded={expanded.has('attr')} onToggle={() => toggle('attr')}>
        <AttributeValuesCard
          ...
          onChange={onUpdate ? (v) => onUpdate((c) => ({ ...c, attributes: Object.keys(v).length ? v : undefined })) : undefined}
          title=""
      ...
      <CollapsibleSection title="规则（每行一条，非玩家控制时按脚本行动）" expanded={expanded.has('rules')} onToggle={() => toggle('rules')}>
        <div style={styles.row}>
          <textarea
      ...
      <CollapsibleSection title="onMeet 属性变更" expanded={expanded.has('onMeetAttr')} onToggle={() => toggle('onMeetAttr')}>
      ...
      <CollapsibleSection title="onMeet 物品变更" expanded={expanded.has('onMeetItems')} onToggle={() => toggle('onMeetItems')}>
        <ItemsEditorCard
          items={items}
...
export function CharacterEditor({ fw, updateFw }: { fw: StoryFramework; updateFw: (fn: (d: StoryFramework) => StoryFramework) => void }) {
  useEffect(() => {
